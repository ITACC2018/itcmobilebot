...
class AccDrivers extends HttpDriver
{
    const DRIVER_NAME = 'AccDrivers';
    ...
    public function getMessages()
    {
        if (empty($this->messages)) {
            ...
            $messageId = $this->event->get('messageId');
            $userName = $this->event->get('userName');
            $userAvatar = $this->event->get('userAvatar');
            $time = $this->event->get('time');
            $status = $this->event->get('status');

            $incomingMessage = new IncomingMessage($message, $sender, $userId, $this->payload, $messageId, $userName, $userAvatar, $time, $status);
        ...
        $replyData = Collection::make($messages)->transform(function ($replyData) {
            ...
            if ($message instanceof WebAccess) {
                ...
                $reply = [
                    ...
                    'messageKey' => $this->event->get('sender') . '_00000',
                    'message' => !empty($message->getText()) ? $message->getText() : 'Maaf, Mr Accbot tidak mengerti maksud anda..Apakah anda ingin chat dengan Team IT Care',
                    'userId' => $this->event->get('userId'),
                    'sender' => $this->event->get('sender'),
                    'senderName' => $this->event->get('senderName'),
                    'messageId' => $this->event->get('messageId'),
                    'userName' => $this->event->get('userName'),
                    'userAvatar' => $this->event->get('userAvatar'),
                    'time' => $this->event->get('time'),
                    'status' => $this->event->get('status'),
                    'takeOver' => !empty($message->getText()) ? 'chatbot-success' : 'chatbot-warning'
                ];
            }
